...
public class ArticleServiceImpl extends ServiceImpl<ArticleMapper, Article> implements ArticleService {
    ...
    public ResponseResult  insertArticle(ArticleDTO article) {
        ...
        // 返回包含userId的文章对象
        ArticleDTO resultDTO = BeanCopyUtils.copyObject(blogArticle, ArticleDTO.class);
        resultDTO.setTags(tagsMapper.selectByArticleId(blogArticle.getId()));
    ...
    public ResponseResult reptile(String url) {
        try {
            ...
            Article entity = Article.builder().userId(7L).contentMd(markdown)
     ...
     */
    @Override
    public ResponseResult selectUserArticleList(Integer categoryId,Integer tagId) {
        Long userId = StpUtil.getLoginIdAsLong();
        Page<ApiArticleListVO> articlePage = baseMapper.selectUserArticleList(new Page<>(PageUtils.getPageNo(), PageUtils.getPageSize()),
               userId, categoryId, tagId);
        articlePage.getRecords().forEach(item -> {
            List<Tags> list = tagsMapper.selectTagByArticleId(item.getId());
            Integer commentCount = commentMapper.selectCount(new LambdaQueryWrapper<Comment>()
                    .eq(Comment::getArticleId, item.getId()));
...
//        List<ArticleSearchVO> articleSearchVOS = searchStrategyContext.executeSearchStrategy(strategy, keywords);
        ...
        articlePage.getRecords().forEach(item -> {
            List<Tags> list = tagsMapper.selectTagByArticleId(item.getId());
            item.setTagList(list);
            item.setTitle(item.getTitle().replaceAll(keywords, Constants.PRE_TAG + keywords + Constants.POST_TAG));
        });
