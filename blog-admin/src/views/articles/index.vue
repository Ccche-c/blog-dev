<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button v-if="canAdd" size="small" class="filter-item" @click="handleAdd" type="primary" icon="el-icon-plus">添加
        </el-button>
      </el-col>
      <el-col :span="1.5">
      ...
      <el-col :span="1.5">
        <el-button size="small" v-if="canUpdate" :disabled="!multipleSelection.length" class="filter-item" @click="seo"
          type="info" icon="el-icon-position">批量SEO
        </el-button>
      </el-col>
      <el-col :span="1.5">
    ...
    <div style="margin-top: 5px">
      <el-table border :data="tableData" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" width="116" label="类型">
          <template slot-scope="scope">
            <el-tag :type="scope.row.isOriginal === 0 ? 'warning' : 'success'" :key="index"
              v-if="scope.row.isOriginal === index" v-for="(item, index) in isOriginalList">{{ item }} </el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="isStick" align="center" width="120" label="置顶">
          <template slot-scope="scope">
      ...
      <el-dialog :title="title" :visible.sync="centerDialogVisible" :before-close="closeModel" fullscreen center>
        <el-form :rules="rules" ref="dataForm" :model="article" style="margin-top: 10px">
          <el-row>
            ...
            <el-col :span="8">
              <el-form-item label="标题图" :label-width="formLabelWidth">
                ...
                <el-col :span="2">
                  <el-upload style="width: 80px;height: 80px" class="avatar-uploader" :show-file-list="false" ref="upload"
                    name="filedatas" :action="uploadPictureHost" :before-upload="uploadBefore"
          ...
          <el-row>
            <el-col :span="6.5">
              <el-form-item label="标签" :label-width="formLabelWidth" prop="tags">
                <el-tag v-for="(item, index) of article.tags" :key="index" style="margin:0 1rem 0 0" :closable="true"
          ...
          <el-row>
            ...
            <el-col :span="5">
              <el-form-item :label-width="formLabelWidth" label="是否首页轮播" prop="isSecret">
            ...
            <el-col :spam="24">
              <el-form-item :label-width="formLabelWidth" label="内容" prop="contentMd">
                <div style="margin-bottom: 10px;">
                  <el-button type="primary" icon="el-icon-magic-stick" size="small" @click="handleAiPush" :loading="aiLoading">
                    AI推送精品内容
                  </el-button>
                </div>
    ...
    <!-- AI推送精品内容对话框 -->
    <el-dialog title="AI推送精品内容" :visible.sync="aiDialogVisible" width="600px">
      <el-form>
        <el-form-item label="提示内容" :label-width="formLabelWidth">
          <el-input 
            ...
            :rows="4" 
            v-model="aiPrompt" 
            placeholder="请输入您希望AI生成的内容提示，例如：基于当前文章标题生成一篇精品文章内容"
            autocomplete="off">
          </el-input>
        </el-form-item>
        <el-form-item label="系统提示词" :label-width="formLabelWidth">
          <el-input 
            v-model="aiSystemPrompt" 
            placeholder="可选：自定义系统提示词，默认为'推送精品内容'"
            autocomplete="off">
          </el-input>
        </el-form-item>
        <el-form-item label="温度参数" :label-width="formLabelWidth">
          <el-slider v-model="aiTemperature" :min="0" :max="2" :step="0.1" show-input></el-slider>
          <div style="font-size: 12px; color: #909399; margin-top: 5px;">
            温度值越低，生成内容越确定；温度值越高，生成内容越随机
          </div>
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        <el-button @click="aiDialogVisible = false">取 消</el-button>
        <el-button :loading="aiLoading" type="primary" @click="confirmAiPush">确 定</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    // 判断是否是普通用户（roleId !== 1 表示非管理员）
    isNormalUser() {
      const roleId = this.roleId
      // roleId = 1 表示管理员，其他都是普通用户
      return roleId !== null && roleId !== undefined && roleId !== 1
    }
  },
  methods: {
    ...
    queryList: function () {
      ...
      console.log('当前角色ID:', this.roleId, typeof this.roleId)
      console.log('是否普通用户 (isNormalUser):', this.isNormalUser)
      
      // 如果是普通用户（roleId !== 1），添加 userId 过滤参数
      if (this.isNormalUser && this.userId) {
        queryParams.userId = this.userId
        console.log('✓ 普通用户，已添加 userId 过滤参数:', this.userId)
      } else {
        if (!this.isNormalUser) {
          console.log('✓ 管理员用户（roleId === 1），不过滤，查看所有文章')
        } else if (!this.userId) {
          console.warn('⚠ 是普通用户但 userId 为空，无法过滤')
        }
      }
      
      console.log('原始查询参数:', this.params)
      console.log('最终查询参数（将发送给后端）:', queryParams)
      console.log('================================\n')
      
      fetchArticle(queryParams).then(res => {
        // 调试：查看返回的文章数据
        console.log('========== 文章查询结果 ==========')
        console.log('返回文章总数:', res.data.total)
        console.log('返回文章列表数量:', res.data.records ? res.data.records.length : 0)
        if (res.data.records && res.data.records.length > 0) {
          console.log('第一篇文章数据字段:', Object.keys(res.data.records[0]))
        }
        console.log('================================\n')
    ...
    resetQuery: function () {
      ...
      // 注意：不要重置 userId，因为它是根据用户角色自动添加的
      this.queryList()
    },
    handleFind: function () {
    ...
    // AI推送精品内容
    handleAiPush: function () {
      // 如果文章有标题，自动填充提示
      if (this.article.title) {
        this.aiPrompt = `基于文章标题"${this.article.title}"，生成一篇精品文章内容`;
      } else {
        this.aiPrompt = '请生成一篇精品文章内容';
      }
      this.aiDialogVisible = true;
    },
    confirmAiPush: function () {
      if (!this.aiPrompt || this.aiPrompt.trim() === '') {
        this.$message.warning('请输入提示内容');
        return;
      }
      ...
      const requestData = {
        ...
        systemPrompt: this.aiSystemPrompt || '推送精品内容',
        temperature: this.aiTemperature || 0
      };
      ...
      aiChat(requestData).then(res => {
        ...
        let aiContent = '';
        if (res.data) {
          // 可能的返回格式：res.data.content, res.data.message, res.data.text, 或直接是 res.data
          aiContent = res.data.content || res.data.message || res.data.text || res.data;
        }
        
        if (aiContent && typeof aiContent === 'string' && aiContent.trim() !== '') {
          // 将AI生成的内容追加到编辑器
          const currentContent = this.article.contentMd || '';
          const newContent = currentContent ? currentContent + '\n\n' + aiContent : aiContent;
          this.article.contentMd = newContent;
          this.$message.success('AI内容已添加到编辑器');
          this.aiDialogVisible = false;
          // 重置表单
          this.aiPrompt = '';
          this.aiSystemPrompt = '推送精品内容';
          this.aiTemperature = 0;
        } else {
          this.$message.warning('AI返回内容为空，请检查接口返回格式');
        }
      }).catch(err => {
        this.aiLoading = false;
        console.error(err);
        this.$message.error('AI推送失败：' + (err.message || '请稍后重试'));
      });
    }
  }
}
